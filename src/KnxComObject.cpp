...
void KnxComObject::getValue(byte value[]) const
{
    for (byte i = 0; i < _dataLength - 1; i++)
        value[i] = _value[i]; // long value case
}
...
{
    for (byte i = 0; i < _dataLength - 1; i++)
        _value[i] = other[i]; // long value case
    _validated = true;
}
...
byte KnxComObject::updateValue(const KnxTelegram &other)
{
    ...
    {
    case 1:
        _value[0] = other.GetFirstPayloadByte();
        break;
    case 2:
        other.GetLongPayload(_value, 1);
        break;
    default:
        other.GetLongPayload(_value, _dataLength - 1);
    }
...
void KnxComObject::copyValue(KnxTelegram &dest) const
{
    ...
    {
    case 1:
        dest.SetFirstPayloadByte(_value[0]);
        break;
    case 2:
        dest.SetLongPayload(_value, 1);
        break;
    default:
        dest.SetLongPayload(_value, _dataLength - 1);
    }
}
