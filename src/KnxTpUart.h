...
// Time out/EOP by telegram recption (ms)
#ifdef ESP32
// ESP32 more time, maybe will be fixed in future
#define KNX_RECEPTION_TIMEOUT 30
#else
// all other MCU are fine with 5 ms
#define KNX_RECEPTION_TIMEOUT 10
#endif

// Definition of the TP-UART working modes
enum type_KnxTpUartMode
{
  NORMAL,
  BUS_MONITOR
};
                                         ...
                                         []) const
{
  ...
  if (_debugStrPtr != NULL)
    *_debugStrPtr += String(_debugInfoText) + String(comment);
#endif
}
                                          ...
                                          []) const
{
  ...
  if (_debugStrPtr != NULL)
    *_debugStrPtr += String(_debugErrorText) + String(comment);
#endif
}
