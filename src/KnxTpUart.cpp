...
{
    if (_tx.state != TX_IDLE)
        return KNX_TPUART_ERROR; // TX not initialized or busy
...
void KnxTpUart::RXTask(void)
{
    ...
    static word lastByteRxTimeMillis;
    ...
    { // a telegram reception is ongoing
        ...
        nowTime = (word)millis(); // word cast because a 65ms looping counter is long enough
        if (TimeDeltaWord(nowTime, lastByteRxTimeMillis) > KNX_RECEPTION_TIMEOUT || telegramCompletelyReceived)
...
void KnxTpUart::TXTask(void)
{
    ...
    case TX_TELEGRAM_SENDING_ONGOING:
        ...
        if (_rx.state != RX_KNX_TELEGRAM_RECEPTION_STARTED && _serial.availableForWrite() >= 2)
        {
...
boolean KnxTpUart::IsAddressAssigned(word addr, byte &index) const
{
    ...
    if (!_assignedComObjectsNb)
        return false; // in case of empty list, we return immediately
