 ...
 * This function call shall be placed in a task with 1 ms cycle time
 */
void KnxDevice::task(void)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (!_initCompleted)
        {
            ...
            {
                while ((_initIndex < _numberOfComObjects) &&
                       (_comObjectsList[_initIndex].getValidity() ||
                        !_comObjectsList[_initIndex].isActive()))
                {
                    _initIndex++;
                }
        ...
        // The TPUART RX task should be executed every 400 us => execute 3 times in 1ms if RTOS task is given
        _tpuart->RXTask();
        ...
        // The TPUART TX task is executed every 800 us => in RTOS every 1 ms
        _tpuart->TXTask();
 ...
 */
template <typename T>
e_KnxDeviceStatus KnxDevice::read(byte objectIndex, T &returnedValue)
{
...
e_KnxDeviceStatus KnxDevice::setComObjectAddress(byte index, word addr, bool active)
{
    if (_state != INIT)
        return KNX_DEVICE_INIT_ERROR;
    if (index >= _numberOfComObjects)
        return KNX_DEVICE_INVALID_INDEX;
...
void KnxDevice::GetTpUartEvents(e_KnxTpUartEvent event)
{
    ...
    if (event == TPUART_EVENT_RECEIVED_KNX_TELEGRAM)
    {
        ...
        case KNX_COMMAND_VALUE_READ:
            ...
            if ((comObj->getIndicator()) & KNX_COM_OBJ_R_INDICATOR)
...
template <typename T>
e_KnxDeviceStatus ConvertFromDpt(const byte dptOriginValue[], T &resultValue, byte dptFormat)
{
    switch (dptFormat)
    {
...
template <typename T>
e_KnxDeviceStatus ConvertToDpt(T originValue, byte dptDestValue[], byte dptFormat)
{
    switch (dptFormat)
    {
    case KNX_DPT_FORMAT_U16:
